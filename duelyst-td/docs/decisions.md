...
## 2026-05-25 — Iter C12: NetController prototype (host-client relay skeleton)
**Decision:** New `scripts/net_controller.gd` (attached as a `Node` child of Main) exposes the C-track networking shape: `host_start(port)` / `client_join(host, port)` / `disconnect_net()` wrap `ENetMultiplayerPeer`; `relay_command(payload)` broadcasts via the `@rpc("any_peer", "call_local", "reliable")` `_rpc_remote_command` so every peer applies the same command. `_apply_remote_command(payload, sender)` whitelists known command types (just `PlaceUnit` in v1 — others are TODO) and reconstructs the `Command` instance with metadata for `origin_slot` + `source_peer_id`. `demo_loopback(unit_id, pos, slot)` and the `--test-net-loopback` CLI flag pump a fake remote command through the same apply path so the relay logic is exercisable headless without two real processes.
**Why:** C12 acceptance says "Create the smallest possible host-client command flow." I can ship the **flow shape** autonomously (ENet wiring + serialization + RPC dispatch + whitelist + loopback test). What I can't ship autonomously is the two-process playtest — that needs the user actually running host and client side by side. This decision documents the v1 cut: skeleton + loopback verified; two-process playtest = follow-up the user drives.
**Impact:** new `scripts/net_controller.gd`, `scenes/main.tscn` (+1 child node), `scripts/main.gd` (`@onready var net`, `--test-net-loopback` CLI flag → `_run_net_loopback_smoke_test()`, F7 hotkey for in-game loopback).
**Test:** `--test-net-loopback` headless run prints `loopback PlaceUnit success=true reason=placed backline_archer (cost 5)` — the command reaches CommandBus and executes successfully from the remote-apply branch. The "no animation named 'idle'" error in that output is a pre-existing UnitFactory edge case on sprites without a named idle frame, unrelated to C12.
